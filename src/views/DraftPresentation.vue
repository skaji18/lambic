<template>
  <v-layout
    v-if="
      ...
      user &&
      (isNewPresentation ||
        (presentation && user.id == presentation.presenter.id))
    "
    row
    class="pb-5"
  >
    <v-flex>
      <v-card class="mb-2">
        ...
        <v-form ref="form" v-model="valid" lazy-validation>
          <v-container fluid class="py-1">
            ...
            <v-layout row class="py-2">
              <v-flex xs12 md7>
                <v-checkbox v-model="checkConfidential" color="green" required>
                  <template #label>
      ...
      <v-btn fixed fab bottom left color="green" @click="backTo">
  ...
  <v-progress-linear
    v-else
    :indeterminate="event !== null && presentation !== null"
  >
  </v-progress-linear>
</template>
<script>
...
export default {
    ...
    return {
      ...
      titleRules: [
        ...
        (v) =>
          ...
          "タイトルは" + this.titleMaxLength + "文字以内にしてください。",
      ],
      descriptionRules: [
        // 発表内容入力規則
        (v) =>
  ...
  created() {
    ...
    const presentation = this.isNewPresentation
      ? null
      : this.$store.getters.presentation(this.id);
    if (presentation) {
  ...
  methods: {
    ...
    submit() {
      ...
      if (
        this.isNewPresentation &&
        confirm("発表を申し込みます。よろしいですか？")
      ) {
        // 発表登録処理
